...
public class EnterRoomAction implements Action, RoomManagerAware, ConnectedUserPoolAware {
	...
	public void doAction(WebSocketSession session, MessageProtocol protocol) throws Exception {
		...
		RoomMemberListResponse roomMemberListResponse = RoomMemberListResponse.of(room.getRoomOwner().toResponse(),
				room.getParticipants().stream().map(e -> RoomMemberListResponse.RoomMemberResponse.of(roomId, e)).collect(Collectors.toList()));
