...
public class LeaveRoomAction implements Action, RoomManagerAware, ConnectedUserPoolAware {
	...
	public void doAction(WebSocketSession session, MessageProtocol protocol) throws Exception {
		...
		pool.remove(session);
		//
		//		RoomMemberListResponse roomMemberListResponse = RoomMemberListResponse.of(room.getRoomOwner()
		//		.toResponse(),
		//				room.getParticipants().stream().map(e -> RoomMemberListResponse.RoomMemberResponse.of(roomId,
		//				e)).collect(Collectors.toList()));

		// 해당 방에 노티
		for (ChatUser participant : room.getParticipants()) {
			String chatMessage = String.format("%s님이 퇴장하셨습니다.", user.getNickname());
			...
			//			participant.getSession().sendMessage(MessageProtocol.of(MessageType.GET_ROOM_MEMBER_LIST,
			//			roomId,
